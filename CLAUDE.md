...
- **Record only on request:** only when he explicitly says **"Record"** (or a clear variant
  ...
  `review`/`weekly`) — `vocab-list.xlsx` is his personal running record of words he chose to
  save. It also feeds the `review` drill (see above) — mixed in with phrase-bank chunks.
